## Substitution analysis for Rice subsp. japonica ##
...
j.150offset <- read.csv("../output/Offset/JaponicaOffset_ur_150_FINAL.csv")
j.150offset$PixelID <- paste(j.150offset$Longitude, j.150offset$Latitude,sep="_")

j.accession <- j.150offset[,c(2:4,7,22)] ##for later in the script 
...
control <- read.csv("../data/GF_data/Japonica_control_FINAL.csv")
...
## GF control model 
gf.Japonica <- readRDS("../output/GF_RDS/GF_Japonica.rds")

# Variables in the model
imp.vars <- names(gf.Japonica$overall.imp)

# Predicted GF model outcomes for control conditions
pred.gf <- predict(gf.Japonica, control.sub[, imp.vars])
pred.gf$PixelID <- control.sub$PixelID 

# Projected GF model outcomes for target (150 Tg) conditions
proj.gf <- predict(gf.Japonica, target.sub[, imp.vars]) 
proj.gf$PixelID <- j.150offset$PixelID 

# For looping through conditions of the worst year (Year 2 post-strike) across pixels
listModL <- colnames(j.150offset[c(7)]) #"offset_7" worst year 
...
for (modL in listModL){ 
  ...
  datModL_prep <- cbind(j.150offset, target.sub[,c("maturity_days_avg")])
  ...
  # Identify the top 55% vulnerable pixels
  getpc <-  quantile(datModL[,4], .45)
...
best_sub_j <- 
  ...
  labs(title = "Rice subsp. japonica",
...
best_grob_j <- ggplotGrob((best_hist_j) +
                            theme(plot.background = element_rect(colour = "black")))

country_grob_j <- ggplotGrob((country_hist_j) +
                               theme(plot.background = element_rect(colour = "black")))
...
j_fig4 <- plot_grid(j_best_plot, j_country_plot, labels = c("(g)", "(h)"))
