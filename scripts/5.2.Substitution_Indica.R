...
##Adapted fromCaproni et al. 2022 DOI: 10.1111/gcb.16560
...
i.150offset <- read.csv("../output/Offset/IndicaOffset_ur_150_FINAL.csv")
i.150offset$PixelID <- paste(i.150offset$Longitude, i.150offset$Latitude,sep="_")

i.accession <- i.150offset[,c(2:4,7,22)] ##for later in the script 
...
control <- read.csv("../data/GF_data/Indica_control_FINAL.csv")
...
## GF control model 
gf.indica <- readRDS("../output/GF_RDS/GF_Indica.rds")

# Variables in the model
imp.vars <- names(gf.indica$overall.imp)

# Predicted GF model outcomes for control conditions
pred.gf <- predict(gf.indica, control.sub[, imp.vars])
pred.gf$PixelID <- control.sub$PixelID 

# Projected GF model outcomes for target (150 Tg) conditions
proj.gf <- predict(gf.indica, target.sub[, imp.vars]) 
proj.gf$PixelID <- i.150offset$PixelID 

# For looping through conditions of the worst year (Year 2 post-strike) across pixels
listModL <- colnames(i.150offset[c(7)]) #"offset_7" worst year 
...
for (modL in listModL){ 
  ...
  datModL_prep <- cbind(i.150offset, target.sub[,c("maturity_days_avg")])
  ...
  # Identify the top 60% vulnerable pixels
  getpc <-  quantile(datModL[,4], .40)
  ...
  for (Pix in selPixel){
    ...
    Dist$country[Dist$PixelID == "100.447778_-1.223333"] <- "Malaysia"
    Dist$country[Dist$PixelID == "109.6425_2.030278"] <- "Malaysia"
    Dist$country[Dist$PixelID == "98.6075_12.42944"] <- "Myanmar"
...
best_grob_i <- ggplotGrob((best_hist_i) +
                            theme(plot.background = element_rect(colour = "black")))

country_grob_i <- ggplotGrob((country_hist_i) +
                               theme(plot.background = element_rect(colour = "black")))
...
i_best_plot <-
  ...
  annotation_custom(
    grob = best_grob_i,
    xmin = 70, xmax = 90,  # Adjust position and size of inset
    ymin = -10, ymax = 12
  )
...
i_fig4 <- plot_grid(i_best_plot, i_country_plot, labels = c("(e)", "(f)"))
