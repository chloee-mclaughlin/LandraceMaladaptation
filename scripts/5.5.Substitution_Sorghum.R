## Substitution analysis for Sorghum ##
...
sorg.150offset <- read.csv("../output/Offset/SorghumOffset_ur_150_FINAL.csv")
sorg.150offset$PixelID <- paste(sorg.150offset$Longitude, sorg.150offset$Latitude,sep="_")

sorg.accession <- sorg.150offset[,c(2:4,7,22)] ##for later in the script 
...
control <- read.csv("../data/GF_data/Sorghum_control_FINAL.csv")
...
for (modL in listModL){ 
  ...
  datModL_prep <- cbind(sorg.150offset, target.sub[,c("maturity_days_avg")])
  colnames(datModL_prep)[23] <- "maturity_days_avg"
  datModL_prep_1 <- datModL_prep %>% filter(maturity_days_avg < 300)
  datModL <- datModL_prep_1[, c("Longitude","Latitude", "PixelID", modL)]
  
  # Get the 25% most vulnerable remaining pixels
  getpc <-  quantile(datModL[,4], .85)
  dat_maxVuln <- datModL[which(datModL[,4] > getpc),] 
  
  # Calc the distance between the 20% most vulnerable pixels for clustering
...
best_sub_s <- 
  ...
  geom_segment(data = temp, aes(x = lon.MinVul, y = lat.MinVul, 
               ...
               arrow = arrow(length = unit(0.1, "inches")), 
               size = 1) +
  coord_fixed(xlim = c(-30, 90), ylim = c(-40,45), expand = FALSE)  + #Sorghum
  theme_bw() + 
...
best_grob_s <- ggplotGrob((best_hist_s) +
                            theme(plot.background = element_rect(colour = "black")))

country_grob_s <- ggplotGrob((country_hist_s) +
                               theme(plot.background = element_rect(colour = "black")))
...
s_best_plot <-
  ...
  annotation_custom(
    grob = best_grob_s,
    xmin = -30, xmax = 5,  # Adjust position and size of inset
    ymin = -40, ymax = 0
  )
...
s_fig4 <- plot_grid(s_best_plot, s_country_plot, labels = c("(i)", "(j)"))
